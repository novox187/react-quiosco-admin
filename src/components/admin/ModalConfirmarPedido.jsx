...
export default function ModalConfirmarPedido() {
    const { setModalConfirmarPedido, setLoadingConfirmarPedidoModal, loadingConfirmarPedidoModal, precioPedido, handleClickCompletarPedido } = useAdmin()

    const [dineroCliente, setDineroCliente] = useState();
    const [vuelto, setVuelto] = useState();

    const handleDineroCliente = (e) => {
        setDineroCliente(e.target.value)
    }
    const calcularVuelto = () => {
        if (dineroCliente > precioPedido.total) {
            let calculo = dineroCliente - precioPedido.total.toFixed(2);
            setVuelto(calculo.toFixed(2));
        } else {
            setVuelto(0);
        }
    }
    ...
    return (
        <div className="flex flex-col w-screen justify-center h-dvh max-w-[40rem] text-white md:py-3 font-raleway bg-transparent ">
            <div className="flex flex-col overflow-y-auto relative bg-zinc-900 rounded-xl p-5 z-20 space-y-5">
                ...
                <h1 className=' w-full text-center'>Cobro del pedido</h1>
